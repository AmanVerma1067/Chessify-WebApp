          ...
          {groupedMoves.length === 0 ? (
            ...
            groupedMoves.map((move, index) => (
              <div key={index} className="flex text-base">
                <span className="w-10 text-gray-400 font-medium">{move.number}.</span>
                <span
                  className={cn(
                    "flex-1 font-mono text-gray-200",
                    currentMove === index * 2 + 1 && "bg-cyan-900 rounded px-2 py-1",
                  )}
                >
                  {move.white?.san}
                </span>
                <span
                  className={cn(
                    "flex-1 font-mono text-gray-200",
                    currentMove === index * 2 + 2 && "bg-cyan-900 rounded px-2 py-1",
                  )}
                >
                  {move.black?.san}
                </span>
